(function(global) {
	...
	function init(config){
		...
		var widgetWrapperEl = document.getElementById('orcidWidget');
		var orcidLogoEl = document.createElement("img");
		orcidLogoEl.src = "https://upload.wikimedia.org/wikipedia/commons/0/06/ORCID_iD.svg";
		orcidLogoEl.width = 20;
		orcidLogoEl.height = 20;
		orcidLogoEl.style = "margin-right: 0.5rem;";
		orcidLogoEl.alt = "ORCID";

		// Create the button element
		if (!signedInOrcid){
			var widgetEl = document.createElement("a");
			widgetEl.id = "orcidLink";
			widgetEl.href = buildReturnUrl();
			widgetEl.title = "Sign in with ORCID";
			var textNode = document.createTextNode(buttonText);
			widgetEl.appendChild(orcidLogoEl);
			widgetEl.appendChild(textNode);
		} else {
			var widgetEl = document.createElement("div");
			widgetEl.textContent = `Signed in as ${signedInIdToken.given_name} ${signedInIdToken.family_name}`;
			widgetEl.title = `ORCID ${signedInIdToken.sub}`;
			widgetEl.appendChild(orcidLogoEl);
		}
		widgetEl.className = "inline-flex p-2 space-x-2 rounded-md text-slate-400 bg-slate-700 hover:bg-slate-900 cursor-pointer";
		// console.log(signedInIdToken, signedInOrcid)
		widgetWrapperEl.appendChild(widgetEl);
	}
